...
void ParticleSystemAVX2::update(float _elapsed)
{
  ...
  for (size_t i = 0; i < m_numParticles; i+=8)
  {
    ...
    switch (movemask8f(cmplteq8f(load8f(&m_particles->m_energy[i]), ZERO)))
    {
    case 0: // f f f f
...
void ParticleSystemAVX2::render()
{
 ...
 auto setVert=[](f256 reg)
 {
   ALIGNED(16)float v[8];
   store8f(v, reg);
    #ifdef GOOGLEBENCH
 ...
 void ParticleSystemAVX2::updatePosition(float _dx, float _dy, float _dz)
 {
    m_pos.m_x+=_dx;
    m_pos.m_y+=_dy;
    m_pos.m_z+=_dz;
